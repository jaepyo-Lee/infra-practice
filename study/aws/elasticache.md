...
클러스터 모드 활성화:
  ...
  │  │ key: a~f   │  │ key: g~m   │  │ key: n~z   │  │
  │  └────────────┘  └────────────┘  └────────────┘  │
  └──────────────────────────────────────────────────┘
  - 여러 샤드(shard)로 데이터를 Hash Slot(0~16383) 기준으로 분산
  - 대용량 데이터(500GB+)에 적합
  - 앱에서 Redis Cluster 프로토콜 지원 필요 (일반 Redis 클라이언트 불가)
```
...
```
레플리카 = 같은 책을 여러 권 복사 (동일 데이터, 복사본)
샤드     = 도서관을 여러 구역으로 나눔 (다른 데이터, 분산 저장)
```

| 개념 | 역할 | 효과 |
|------|------|------|
| 레플리카 | Primary 데이터를 동기 복제 | HA(장애복구) + 읽기 분산 |
| 샤드 | 데이터를 분산 저장하는 단위 | 용량/쓰기 수평 확장 |
| 클러스터 모드 | 여러 샤드를 묶는 구성 | 샤드 기능을 활성화 |

**레플리카의 두 가지 역할**

```
1. HA: Primary 장애 시 Replica가 자동 승격 → 다운타임 최소화
2. 읽기 분산: Reader 엔드포인트로 GET/SELECT를 Replica로 분산

App → Primary 엔드포인트 → 쓰기 (SET, DEL 등)
App → Reader 엔드포인트  → 읽기 (GET, SCAN 등)
```
...
| 상황 | 선택 |
|------|------|
| 데이터 < 500GB, 일반 서비스 | 비활성화 (이 프로젝트) |
| 데이터 > 500GB, 대용량 서비스 | 활성화 |
| 단일 Primary 쓰기 처리량 한계 도달 | 활성화 |
